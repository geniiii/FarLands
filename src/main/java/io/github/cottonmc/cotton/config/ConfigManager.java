...
public class ConfigManager {
	...
	public static <T> T loadConfig(Class<T> clazz, String configName, Jankson jankson) {
		try {
			...
			try {
				...
				FarLands.getLogger().warn("Failed to load config File " + configName + CONFIG_FILE_EXTENSION + ": ", e);
			}
		} catch (SyntaxError syntaxError) {
			FarLands.getLogger().warn("Failed to load config File " + configName + CONFIG_FILE_EXTENSION + ": ", syntaxError);
		} catch (IllegalAccessException | InstantiationException e) {
			FarLands.getLogger().warn("Failed to create new config file for " + configName + CONFIG_FILE_EXTENSION + ": ", e);
		}

		//Something obviously went wrong, create placeholder config
		FarLands.getLogger().warn("Creating placeholder config for " + configName + CONFIG_FILE_EXTENSION + "...");
		try {
			return clazz.newInstance();
		} catch (InstantiationException | IllegalAccessException e) {
			FarLands.getLogger().warn("Failed to create placeholder config for " + configName + CONFIG_FILE_EXTENSION + ": ", e);
		}
	...
	public static void saveConfig(Object object, String configName, Jankson jankson) {
		...
		try {
			...
			FarLands.getLogger().warn("Failed to write to config file" + configName + CONFIG_FILE_EXTENSION + ": " + e);
		}
	}
